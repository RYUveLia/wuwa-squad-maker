...
const OWNED_MODAL_STYLES = {
  header: 'flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between pb-3 border-b border-[#262630] flex-shrink-0',
  headerTitleArea: 'flex flex-col',
  title: 'text-base font-bold text-zinc-100',
  description: 'text-[10px] sm:text-xs text-zinc-500 mt-0.5',
  headerBtnArea: 'flex gap-2 self-end sm:self-auto shrink-0',
  shortcutBtn: `${COMMON_STYLES.subBtn} whitespace-nowrap shrink-0`,
  grid: 'grid grid-cols-4 sm:grid-cols-5 gap-x-3 gap-y-4 sm:gap-x-4 sm:gap-y-5 overflow-y-auto flex-1 py-5 px-2 pr-1 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent justify-items-center',
  ...
  nameText: 'text-[9.5px] sm:text-[10.5px] font-bold text-zinc-300 group-hover:text-white truncate block px-0.5',
  checkBadge: 'absolute top-1 right-1 bg-amber-400 text-zinc-950 rounded-full w-4 h-4 flex items-center justify-center shadow border border-amber-300 select-none pointer-events-none scale-90 sm:scale-100 animate-scale-up',
  checkText: 'text-[10px] font-black leading-none',
  footer: 'flex items-center justify-between pt-3 border-t border-[#262630] flex-shrink-0',
...
const CARD_CLASS = (isOwned: boolean, rarity: number) => {
  const rarityBorder = rarity === 5
    ? isOwned ? 'border-amber-400 shadow-amber-400/20' : 'border-[#262630]'
    : isOwned ? 'border-violet-400 shadow-violet-500/10' : 'border-[#262630]'
  ...
  return `relative w-full max-w-[64px] sm:max-w-[76px] md:max-w-[80px] h-[86px] sm:h-[102px] md:h-[108px] rounded-xl overflow-hidden border flex flex-col items-center justify-between p-1 cursor-pointer transition-all duration-200 select-none group hover:scale-[1.03] flex-shrink-0 ${
    isOwned ? 'opacity-100 bg-[#14141a] shadow-md' : 'opacity-40 bg-[#09090d]/40 grayscale hover:opacity-75 hover:grayscale-0'
  } ${rarityBorder}`
}
